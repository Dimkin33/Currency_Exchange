...
class MissingRequiredFieldsError(TemplateAPIError):
    ...
    template: str = field(init=False, default="Отсутствует нужное поле формы: {fields}", repr=False)


@dataclass
class MissingCurrencyCodeError(TemplateAPIError):
    status: int = field(init=False, default=400, repr=False)
    template: str = field(init=False, default="Currency code is required", repr=False)
